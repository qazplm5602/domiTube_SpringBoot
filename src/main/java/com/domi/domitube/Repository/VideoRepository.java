...
public interface VideoRepository extends JpaRepository<Video, Object> {
    ...
    @Query("SELECT c FROM Video c WHERE c.owner.id IN :users AND c.secret = :secret ORDER BY c.created DESC")
    List<Video> GetVideoByUsersLatest(@Param("users") List<String> users, @Param("secret") Video.VideoSecretType secret, Pageable pageable);
}
